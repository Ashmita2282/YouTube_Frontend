...
export const useFetchVideos = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const response = await fetch("https://youtube-backend-1-negc.onrender.com/api/video");
        if (!response.ok) {
...
export const useFetchVideosbyId = (id) => {
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        const response = await fetch(`https://youtube-backend-1-negc.onrender.com/api/video/${id}`);
        if (!response.ok) {
          if (response.status === 404) {
            // Handle 404 silently
    ...
    const fetchVideos = async () => {
      ...
      try {
        const response = await fetch(
          `https://youtube-backend-1-negc.onrender.com/api/channel/${channelId}`,
          {
            method: "GET",
            headers: {
